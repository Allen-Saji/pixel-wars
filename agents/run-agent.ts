...
const TEAM_COLORS: Record<number, [number, number, number]> = {
...
if (!idlPath) { console.error("Cannot find idl.json"); process.exit(1); }
...
function findCanvasPDA(round: number): PublicKey {
...
function findAgentRegistrationPDA(agentPk: PublicKey, round: number): PublicKey {
  const [pda] = PublicKey.findProgramAddressSync(
    [Buffer.from("agent"), agentPk.toBuffer(), toLeU32(round)],
    PROGRAM_ID
  );
  return pda;
}
...
async function main() {
  ...
  const configAccount = await (l1Program.account as any)["gameConfig"].fetch(configPDA);
  const round = configAccount.currentRound as number;
  const roundActive = configAccount.roundActive as boolean;
