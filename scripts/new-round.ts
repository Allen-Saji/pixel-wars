/**
 * End current round (if active) and start a fresh one.
 * Usage: NODE_OPTIONS="--dns-result-order=ipv4first" npx tsx scripts/new-round.ts
 */
import { PublicKey, SystemProgram, ComputeBudgetProgram, Connection, Keypair, Transaction, TransactionInstruction } from "@solana/web3.js";
import fs from "fs";
...
async function sendIx(conn: Connection, kp: Keypair, programId: PublicKey, keys: any[], data: Buffer, preIxs?: TransactionInstruction[]) {
  const ix = new TransactionInstruction({ programId, keys, data });
  const tx = new Transaction();
  if (preIxs) preIxs.forEach((pi) => tx.add(pi));
  tx.add(ix);
...
async function main() {
  ...
  if (active) {
    ...
    if (canvasInfo?.owner.toBase58() === DELEGATION_PROGRAM.toBase58()) {
      ...
      for (let i = 0; i < 15; i++) {
        ...
        if (info && info.owner.toBase58() === PROGRAM_ID.toBase58()) { console.log("Canvas back!"); break; }
  ...
  // Start round (creates small canvas)
  ...
  const [bufferPda] = PublicKey.findProgramAddressSync([Buffer.from("buffer"), canvasPDA.toBuffer()], DELEGATION_PROGRAM);
  const [delegationRecordPda] = PublicKey.findProgramAddressSync([Buffer.from("delegation-record"), canvasPDA.toBuffer()], DELEGATION_PROGRAM);
  const [delegationMetadataPda] = PublicKey.findProgramAddressSync([Buffer.from("delegation-metadata"), canvasPDA.toBuffer()], DELEGATION_PROGRAM);
  ...
  try {
    ...
    console.log("Delegation error:", e.message?.slice(0, 200));
  }
